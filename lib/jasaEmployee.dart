...
class ServiceCheckInOutPage extends StatefulWidget {
  final String jobDesk;
  final Map<String, dynamic> userData;

  ServiceCheckInOutPage({required this.jobDesk, required this.userData});

  @override
  _ServiceCheckInOutPageState createState() => _ServiceCheckInOutPageState();
}

class _ServiceCheckInOutPageState extends State<ServiceCheckInOutPage> {
  ...
  void scheduleNotifications(DateTime checkInTime, String shift) {
    ...
    } else if (shift == '6661d7e86ac14ec27d2b957d') {
      // Shift Night
      DateTime nextDay = checkInTime.add(Duration(days: 1));
      notificationTimes = [
        DateTime(nextDay.year, nextDay.month, nextDay.day, 7, 45),
        DateTime(nextDay.year, nextDay.month, nextDay.day, 8, 00),
      ];
    }
  ...
  void _submitAttendance(bool isCheckOut) async {
    ...
    if (isCheckOut) {
      ...
      data['userRole'] = 'Service Employees'; // Sesuaikan dengan role yang digunakan
      ...
      } else if (_selectedShift == '6661d7e86ac14ec27d2b957d') {
        // Shift Night - Check-out otomatis pukul 08:00 berikutnya
        DateTime tomorrow = DateTime(now.year, now.month, now.day + 1, 8, 0, 0);
        int secondsUntilCheckOut = tomorrow.difference(now).inSeconds;
        _timer = Timer(Duration(seconds: secondsUntilCheckOut), () {
          _stopClock();
          _resetClock();
          _submitAttendance(true); // Lakukan check-out otomatis
                      ...
                      children: <Widget>[
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          items: [
                            ...
                            DropdownMenuItem<String>(
                              value: '6661d7e86ac14ec27d2b957d',
                              child: Text('Night'),
                            ),
                          ],
                          onChanged: (value) {
